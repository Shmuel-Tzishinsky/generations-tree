class Node {
  constructor({ id, name, gender, partner = [], father, mother, born, deceased, width, height, details }) {
    this.id = id;
    this.name = name;
    this.prelimX = 0;
    this.prelimY = 0;
    ...
    this.born = !isNaN(born) ? born : null;
    this.deceased = deceased || null;
    this.modifier = 0;
  ...
  getDescendent(id, name) {
    ...
    if (this.getChild(id)) {
      ...
      for (let i = 0; i < children.length; i++) {
        found = children[i].getDescendent(id, name);
        if (found) {
          return found;
        }
      }
